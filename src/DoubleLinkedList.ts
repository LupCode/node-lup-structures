...
type DoubleLinkedListOptions<T> = {
  /**
...
export default class DoubleLinkedList<T> {
  ...
  private readonly _options: DoubleLinkedListOptions<T> | undefined;
   ...
   */
  constructor(options?: DoubleLinkedListOptions<T>, values?: T[]) {
   ...
   */
  public concat(...other: DoubleLinkedList<T>[]): DoubleLinkedList<T> {
    const newList = new DoubleLinkedList<T>(this._options, this.toArray());
   ...
   */
  public concatInPlace(...other: DoubleLinkedList<T>[]): this {
    for (const list of other) {
      for (const value of list) {
        this.push(value);
      }
   ...
   */
  public filter<S>(predicate: (value: T, index: number, list: this) => S, thisArg?: any): DoubleLinkedList<T> {
   ...
   */
  public flat(depth: number = 1): DoubleLinkedList<T> {
    const newList = new DoubleLinkedList<T>();
   ...
   */
  public flatMap<S>(callback: (value: T, index: number, list: this) => S, thisArg?: any): DoubleLinkedList<S> {
    return this.map<S>(callback, thisArg).flat() as DoubleLinkedList<S>;
  }
   ...
   * @param equals Optional equality function to use for comparison.
   * @return Found value, or undefined if the value was not found.
   */
  public getByComparator(lookup: T, equals?: (a: T, b: T) => boolean): T | undefined {
    const _equals = equals ?? this._equals;
    let currNode = this._head;
    while (currNode) {
      if (_equals(currNode.value, lookup)) {
        return currNode.value;
      }
      currNode = currNode.next;
    }
   ...
   * @return Found value, or undefined if the value was not found.
   */
  public getLastByComparator(lookup: T, equals?: (a: T, b: T) => boolean): T | undefined {
    const _equals = equals ?? this._equals;
   ...
   */
  public includes(lookup: T, fromIndex?: number, equals?: (a: T, b: T) => boolean): boolean {
    fromIndex = fromIndex ?? 0;
    let currNode = this._head;
    let index = 0;
    const _equals = equals ?? this._equals;
    while (currNode) {
      if (index >= fromIndex && _equals(currNode.value, lookup)) {
        return true;
      }
      currNode = currNode.next;
      index++;
   ...
   */
  public indexOf(lookup: T, fromIndex?: number, equals?: (a: T, b: T) => boolean): number {
    fromIndex = fromIndex ?? 0;
    let currNode = this._head;
    let index = 0;
    const _equals = equals ?? this._equals;
  ...
  public join(separator?: string): string {
    ...
    while (currNode) {
      result +=
        (result.length > 0 ? separator : '') +
        (currNode.value?.toString ? currNode.value.toString() : String(currNode.value));
      currNode = currNode.next;
    }
    return result;
  }
   ...
   * @return Index of the found value, or -1 if the value was not found.
   */
  public lastIndexOf(lookup: T, fromIndex?: number, equals?: (a: T, b: T) => boolean): number {
   ...
   * @param thisArg — An object to which the this keyword can refer in the callbackfn function. If thisArg is omitted, undefined is used as the this value.
   * @return A new DoubleLinkedList with each element being the result of the callback function.
   */
  public map<U>(callbackfn: (value: T, index: number, list: this) => U, thisArg?: any): DoubleLinkedList<U> {
    const newList = new DoubleLinkedList<U>();
    let currNode = this._head;
    let index = 0;
    while (currNode) {
   ...
   */
  public reduce<U = T>(
    callbackfn: (previousValue: U, currentValue: T, currentIndex: number, list: this) => U,
    initialValue?: U,
  ): U {
    let currNode = this._head;
    let index = 0;
   ...
   */
  public remove(lookup: T, equals?: (a: T, b: T) => boolean): T | undefined {
    if (!this._head) return undefined;
    const _equals = equals ?? this._equals;

    // special case for head
    if (_equals(this._head.value, lookup)) {
    ...
    let currNode = this._head;
    while (currNode.next) {
      if (_equals(currNode.next.value, lookup)) {
   ...
   */
  public removeAll(lookup: T, equals?: (a: T, b: T) => boolean): number {
    const _equals = equals ?? this._equals;
    let removedCount = 0;

    // remove from head
    while (this._head && _equals(this._head.value, lookup)) {
      this._head = this._head.next;
      if (this._head) {
        this._head.prev = null;
      }
   ...
   * @return A new DoubleLinkedList containing the extracted elements.
   */
  public slice(start?: number, end?: number): DoubleLinkedList<T> {
    start = start === undefined ? 0 : start < 0 ? this._size + start : start;
    end = end === undefined ? this._size : end < 0 ? this._size + end : end;
    if (start < 0) start = 0;
    if (end > this._size) end = this._size;
  ...
  public sort(compareFn?: (a: T, b: T) => number): this {
     ...
     */
    function merge<U>(
      a: DoubleLinkedListNode<U> | null,
      b: DoubleLinkedListNode<U> | null,
      compare: (x: U, y: U) => number,
    ): { head: DoubleLinkedListNode<U> | null; tail: DoubleLinkedListNode<U> | null } {
   ...
   */
  public splice(start: number, deleteCount?: number, ...items: T[]): DoubleLinkedList<T> {
    start = start < 0 ? Math.max(this._size + start, 0) : Math.min(start, this._size);
    deleteCount =
      deleteCount === undefined ? this._size - start : Math.max(Math.min(deleteCount, this._size - start), 0);
    const removedList = new DoubleLinkedList<T>();
  ...
  public toLocaleString(maxElements: number = MAX_ELEMENTS_FOR_TO_STRING): string {
    ...
    for (let i = 0; i < maxElements && currNode; i++) {
      result +=
        (result.length > 0 ? ', ' : '') +
        (currNode.value?.toLocaleString ? currNode.value.toLocaleString() : String(currNode.value));
      currNode = currNode.next;
    }
    return 'DoubleLinkedList[ ' + result + (this._size > maxElements ? ', ...' : '') + ' ]';
  }
