...
export type SortedMapOptions = {
  /** Initial capacity of the map how many lots it should hold at the beginning (default 16). */
  initialCapacity?: number;

  /** Factor/multiplier by which the capacity of the map should be increased when it is full (default 2). */
  scaleFactor?: number;
   ...
   * @param a First key.
   * @param b Second key.
   * @returns Negative number if a < b, positive number if a > b, zero if a === b.
   */
...
export class SortedMap<K, V> {
  ...
  private readonly compare: (a: any, b: any) => number;
    ...
    return {
      next(): IteratorResult<[K, V]> {
        if (idx < self.size) {
          ...
          const value = self.myValues[idx] as V;
          idx++;
          if (k < key || (inclusive && k === key)) return { value: [k, value], done: false };
        }
        return { value: undefined, done: true };
      },
    ...
    return {
      next(): IteratorResult<K> {
        if (idx < self.size) {
          const k = self.myKeys[idx] as K;
          idx++;
          if (k < key || (inclusive && k === key)) return { value: k, done: false };
        }
        return { value: undefined, done: true };
      },
    ...
    return {
      next(): IteratorResult<V> {
        if (idx < self.size) {
          ...
          const value = self.myValues[idx] as V;
          idx++;
          if (k < key || (inclusive && k === key)) return { value, done: false };
        }
        return { value: undefined, done: true };
      },
