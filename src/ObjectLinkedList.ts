 ...
 */
export class ObjectLinkedList<T extends LinkedListComparator<T>> {
     ...
     */
    public concat(...other: ObjectLinkedList<T>[]): ObjectLinkedList<T> {
        const newList = new ObjectLinkedList<T>(this.toArray());
     ...
     */
    public concatInPlace(...other: ObjectLinkedList<T>[]): this {
        for(const list of other){
            for(const value of list){
                this.push(value);
            }
     ...
     */
    public filter<S>(predicate: (value: T, index: number, list: this) => S, thisArg?: any): ObjectLinkedList<T> {
        const newList = new ObjectLinkedList<T>();
        let currNode = this._head;
        let index = 0;
        while(currNode){
            if(predicate.call(thisArg, currNode.value, index, this)){
     ...
     */
    public flat(depth: number = 1): ObjectLinkedList<T> {
        const newList = new ObjectLinkedList<T>();
     ...
     */
    public flatMap<S extends LinkedListComparator<S>>(callback: (value: T, index: number, list: this) => S, thisArg?: any): ObjectLinkedList<S> {
        return this.map<S>(callback, thisArg).flat() as ObjectLinkedList<S>;
    }
     ...
     * @param thisArg — An object to which the this keyword can refer in the callbackfn function. If thisArg is omitted, undefined is used as the this value.
     * @return A new ObjectLinkedList with each element being the result of the callback function.
     */
    public map<U extends LinkedListComparator<U>>(callbackfn: (value: T, index: number, list: this) => U, thisArg?: any): ObjectLinkedList<U> {
        const newList = new ObjectLinkedList<U>();
        let currNode = this._head;
        let index = 0;
        while(currNode){
     ...
     * @return A new ObjectLinkedList containing the extracted elements.
     */
    public slice(start?: number, end?: number): ObjectLinkedList<T> {
        start = start === undefined ? 0 : (start < 0 ? this._size + start : start);
        end = end === undefined ? this._size : (end < 0 ? this._size + end : end);
        if(start < 0) start = 0;
        if(end > this._size) end = this._size;
     ...
     */
    public splice(start: number, deleteCount?: number, ...items: T[]): ObjectLinkedList<T> {
        start = start < 0 ? Math.max(this._size + start, 0) : Math.min(start, this._size);
                ...
                const removedList = new ObjectLinkedList<T>();
    ...
    public toLocaleString(maxElements: number = MAX_ELEMENTS_FOR_TO_STRING): string {
        ...
        return 'ObjectLinkedList[ ' + result + (this._size > maxElements ? ', ...' : '') + ' ]';
    }
