...
/**
 * High-performance single linked list with user-defined equality function.
 */
export default class LinkedList<T> {
  private _size: number = 0;
  private _head: LinkedListNode<T> | null = null;
  private _tail: LinkedListNode<T> | null = null;
  private readonly _options: LinkedListOptions<T> | undefined;
  private readonly _equals: (a: T, b: T) => boolean;

  /**
   * Creates a new linked list.
   * @param options Options for the linked list.
   * @param values Optional initial values to add to the linked list.
   */
  constructor(options?: LinkedListOptions<T>, values?: T[]) {
   ...
   */
  public concat(...other: LinkedList<T>[]): LinkedList<T> {
    const newList = new LinkedList<T>(this._options, this.toArray());
   ...
   */
  public concatInPlace(...other: LinkedList<T>[]): this {
    for (const list of other) {
      for (const value of list) {
        this.push(value);
      }
   ...
   */
  public filter<S>(predicate: (value: T, index: number, list: this) => S, thisArg?: any): LinkedList<T> {
  ...
  public flat(depth: number = 1): LinkedList<T> {
    const newList = new LinkedList<T>();
    const flatten = (node: LinkedListNode<T> | null, currentDepth: number) => {
      while (node) {
        if (Array.isArray(node.value)) {
          for (const item of node.value) {
            flatten({ value: item, next: null }, currentDepth - 1);
          }
        } else {
  ...
  /*public flatInPlace(depth: number = 1): this {
        ...
        const flattenInPlace = (curr: LinkedListNode<any>, currentDepth: number): [LinkedListNode<any>, LinkedListNode<any>]  => {
   ...
   */
  public flatMap<S>(callback: (value: T, index: number, list: this) => S, thisArg?: any): LinkedList<S> {
    return this.map<S>(callback, thisArg).flat() as LinkedList<S>;
  }
   ...
   * @param thisArg — An object to which the this keyword can refer in the callbackfn function. If thisArg is omitted, undefined is used as the this value.
   * @return A new LinkedList with each element being the result of the callback function.
   */
  public map<U>(callbackfn: (value: T, index: number, list: this) => U, thisArg?: any): LinkedList<U> {
   ...
   * @return A new LinkedList containing the extracted elements.
   */
  ...
  public sort(compareFn?: (a: T, b: T) => number): this {
     ...
     */
    function merge<U>(
      a: LinkedListNode<U> | null,
      b: LinkedListNode<U> | null,
      compare: (x: U, y: U) => number,
    ): { head: LinkedListNode<U> | null; tail: LinkedListNode<U> | null } {
  ...
  public splice(start: number, deleteCount?: number, ...items: T[]): LinkedList<T> {
    ...
    const removedList = new LinkedList<T>();
    ...
    if (start === 0) {
      ...
      for (let i = 0; i < deleteCount && this._head; i++) {
        const nextNode: LinkedListNode<T> | null = this._head.next;
    ...
    for (let i = 0; i < deleteCount && currNode.next; i++) {
      const curr: LinkedListNode<T> = currNode.next;
      const nextNode: LinkedListNode<T> | null = currNode.next.next;
  ...
  public toLocaleString(maxElements: number = MAX_ELEMENTS_FOR_TO_STRING): string {
    ...
    return 'LinkedList[ ' + result + (this._size > maxElements ? ', ...' : '') + ' ]';
  }
