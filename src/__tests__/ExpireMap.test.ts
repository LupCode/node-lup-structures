...
describe('Testing ExpireMap', () => {
  ...
  test('Constructor with values', async () => {
    const map = new ExpireMap({ defaultMaxAge: 50 }, [
      ['a', 1],
      ['b', 2],
    ]);
  ...
  test('entries with timeout', async () => {
    ...
    for (let pair of map.entries()) {
      ...
      switch (i) {
        case 0:
          expect(pair[0]).toEqual('a');
          expect(pair[1]).toEqual(1);
          break;
        case 1:
          expect(pair[0]).toEqual('b');
          expect(pair[1]).toEqual(2);
          break;
        case 2:
          expect(pair[0]).toEqual('c');
          expect(pair[1]).toEqual(3);
          break;
        default:
    ...
    map.forEach((v: number, k: string) => {
      switch (i) {
        ...
        case 2:
          ...
          break;
        default:
          throw new Error('Unexpected key-value-pair ' + [k, v]);
      }
      i++;
    });
    expect(i).toEqual(3);
  ...
  test('resetAge: manual', async () => {
    const map = new ExpireMap({ defaultMaxAge: 50, resetLifespanOnAccess: false }, [
      ['a', 1],
      ['b', 2],
    ]);
    map.set('c', 3, 1000);
  ...
  test('resetLifespanOnAccess: get()', async () => {
    const map = new ExpireMap({ defaultMaxAge: 50, resetLifespanOnAccess: true }, [
      ['a', 1],
      ['b', 2],
    ]);
    map.set('c', 3, 1000);
