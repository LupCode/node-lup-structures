...
export class ExpireMap<K, V> {
    ...
    private _findIndexFirst(timestamp: number): number {
        ...
        while(low < high){
            ...
            if(cmp <= 0){ // special to deal with duplicates
                high = mid;
            } else {
                low = mid + 1;
            }
    ...
    private _cleanup(){
        if(this._expireTimes.length === 0)
            return;

        const now = Date.now();
        let i = 0;
        while(i < this._expireTimes.length){
            if((this._expireTimes[i] as number) > now)
                break;
    ...
    public *entries(): IterableIterator<[K, V]> {
        ...
        for(const [key, info] of this._map.entries()){
            ...
            if(info.expireTime !== undefined && Date.now() >= info.expireTime)
                continue;
    ...
    public resetAge(key: K, maxAge?: number | null): boolean {
        ...
        const lifespan = maxAge === undefined ? info.lifespan : (maxAge === null ? this._defaultMaxAge : (maxAge > 0 ? maxAge : undefined));
    ...
    public toDebugString(): string {
        ...
        result += "  map=[\n";
        for(const [key, info] of this._map.entries()){
            result += "    "+key+" = "+JSON.stringify({ ...info, expired: info.expireTime !== undefined && Date.now() >= info.expireTime })+"\n";
        }
        result += "  ]\n";
